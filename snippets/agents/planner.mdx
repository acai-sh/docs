...
* [ ] Prepare task and write it to a file in `feat/{feature-branch}`:
    1. Identify what our new task id will be e.g. `FEAT.1`
    2. Create `/features/{feature_name}/tasks/{task_id}.md`
    3. Commit the new task file and progress tracker changes to the feature branch
* [ ] Checkout a new working task branch `task/{task_id}`
* [ ] Report back to the supervisor: "I have prepared task id: `{task_id}`, updated the progress tracker, and checked us out to a working task branch. Feel free to proceed with assignment."
