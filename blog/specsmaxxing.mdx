...
And remember, nothing beats an organic, pasture-raised, hand-written spec. **Spec-writing is, in my opinion, where the act of *software engineering* really happens**.

So a few weeks ago, I started asking myself, how far can I take this? How far *should* I take this?

## Dreaming in markdown

As the story goes, I fell into an AI psychosis, I became a “spec maxi”, and I spent hours and hours writing the most beautiful PRDs and TRDs you’ve ever seen.
...
Exciting. But what did that agent army ship for me? Well, it wasn't slop, in fact it worked better than some of the garbage software I'm forced to use every day. 

But it was still a bit sloppy. This wasn't good enough for me.

One hallmark symptom of AI psychosis is *using AI to build things that help AI build products*, rather than just using AI to build the damn product. I embraced my illness, threw out the branch, and rewrote my markdown, and started all over again.
...
The spec is what you *want* the software to be. Very often the spec is not written down. It exists only in your head or in conversations. We used to spend our time writing down procedures (as code), or writing down invariants (as unit tests), and more recently we've been writing out deltas or changesets to evolve our software (as prompts).

But when the procedures and invariants become cheap and invisible, the spec still lives *somewhere*, and **humans will always care what the spec says**. Businesses and product teams will always care when the spec changes. 

So you're better off writing it down now! And I propose that a plain old list of acceptance criteria is a good place to start. (That's really all that `feature.yaml` is.)

A few more things follow from there;

### -> From Specsmaxxing to Testmaxxing

When your spec is defined and the code is generated faster than you can read it, the bottleneck is purely QA and validation and your *confidence* in shipping. So investing heavily in QA automation and test coverage has a massive ROI.

And after that, after maximal coverage and observability has been achieved, our attention shifts away from the diffs in GitHub; we need a place to relate the specs to the artifacts that were generated from them (the implementations, QA feedback, user feedback, builds, runs, reviews and reports). Maybe Acai.sh can become that place. For now, it is only focused on the specs and their implementations in code.
...
I have some rudimentary versions of this, but they aren't ready to share yet.
...
- Acai requires you to adopt the **feature.yaml** format. Almost zero learning curve. I have written an [introductory guide](/spec-driven-development) to writing them. Use `npx @acai.sh/cli skill` to teach your agent how. I also wrote a [spec-for-the-spec](/feature-yaml) if you want to go deeper.
